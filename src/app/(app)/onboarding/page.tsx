  ...
  async function handleSave() {
    ...
    try {
      await setDoc(doc(db, `users/${user.uid}`), {
        ...
        onboardingText: text,          // stored for future generation prompts
        settings: {
          theme:           'warm-kitchen',
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'review' && (
          ...
          >
            ...
            <p className="text-on-surface-muted mb-6">Adjust archetypes, diet, and cuisines.</p>
            ...
            <div className="space-y-4 mb-6">
              {members.map((m, idx) => (
                <motion.div
                  ...
                  className="rounded-2xl bg-surface-raised border border-border p-4 space-y-3"
                >
                  {/* Name */}
                  ...
                  {/* Archetype */}
                  <div>
                    <p className="text-xs font-medium text-on-surface-muted mb-1.5">Goal / Health</p>
                  ...
                  {/* Diet (veg / non-veg) */}
                  <div>
                    <p className="text-xs font-medium text-on-surface-muted mb-1.5">Diet</p>
                      ...
                      {DIET_OPTIONS.map(d => (
                        <button
                          ...
                          className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                            m.hardConstraints.diet === d.value
                              ? 'bg-brand text-white'
                              : 'bg-surface border border-border text-on-surface-muted hover:border-brand/50'
                          }`}
                        >
                  ...
                  {/* Allergy tags */}
                  {m.hardConstraints.allergies.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {m.hardConstraints.allergies.map(a => (
                        <span key={a} className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-600">⚠️ {a}</span>
                      ))}
                    </div>
                  )}
                </motion.div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 rounded-2xl border border-border py-3 text-on-surface-muted font-medium"
              >
                ← Back
              </button>
              <button
                onClick={handleSave}
                disabled={loading}
                className="flex-1 rounded-2xl bg-brand px-8 py-3 text-white font-semibold disabled:opacity-50"
              >
